...
COLUMNS = ["year1", "year2", "playerID", "wins", "losses", "games", "outs_pitched",
			"hits", "earned_runs", "strike_outs", "walks", "opponent_batting_avg",
			"league_batting_avg","earned_run_avg_year1", "earned_run_avg_year2"]

CONTINUOUS_COLUMNS = ["wins", "losses", "games", "outs_pitched", "hits", "earned_runs",
						"strike_outs", "walks", "opponent_batting_avg",
...
def predicciones(dataset, resultados, error, salida, boolean):
	...
	aux = 0
	for i in resultados:
		linea = "Jugador: {0} | Año: {1} | Prediccion: {3}".format(dataset["playerID"][aux],dataset["year2"][aux],dataset["games"][aux],i)
		if boolean:
...
def main():
	...
	wins = tf.contrib.layers.real_valued_column("wins")
	losses = tf.contrib.layers.real_valued_column("losses")
	games = tf.contrib.layers.real_valued_column("games")
	outs_pitched = tf.contrib.layers.real_valued_column("outs_pitched")
	hits = tf.contrib.layers.real_valued_column("hits")
	earned_runs = tf.contrib.layers.real_valued_column("earned_runs")
	strike_outs = tf.contrib.layers.real_valued_column("strike_outs")
	walks = tf.contrib.layers.real_valued_column("walks")
	...
	m = tf.contrib.learn.LinearRegressor(feature_columns=[wins, losses, games, outs_pitched, hits,
			 earned_runs, strike_outs, walks, opponent_batting_avg, league_batting_avg,
			 earned_run_avg_year1], optimizer=tf.train.FtrlOptimizer(learning_rate=alpha,
    		 l1_regularization_strength=1, l2_regularization_strength=1), model_dir=model_dir)
